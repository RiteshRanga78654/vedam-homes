 ...
 *
 * NOTE: Vercel's production filesystem is READ-ONLY except for /tmp.
 * All fs operations below are wrapped in try/catch so that a failed
 * write (e.g. in production) never crashes the request — it just
 * logs a warning and the app keeps working with in-memory/seed data
 * for that request.
 */
...
function ensureDir(dir) {
  try {
    ...
    console.error(`[store] ensureDir(${dir}) failed:`, err.message);
    return false;
  }
}
...
function readJson(name) {
  try {
    ...
    console.error(`[store] readJson(${name}) failed:`, err.message);
    return null;
  }
}
...
function writeJson(name, data) {
  try {
    if (!ensureDir(STORE_DIR)) return false;
    const file = storePath(name);
    fs.writeFileSync(file, JSON.stringify(data, null, 2), "utf8");
    return true;
  } catch (err) {
    // Read-only filesystem in production (Vercel) lands here.
    // Don't throw — caller keeps working with in-memory data.
    console.error(`[store] writeJson(${name}) failed (read-only fs?):`, err.message);
    return false;
  }
}
...
function seedGallery() {
  return siteGallery.map((g, i) => ({
    id: `g-${i + 1}`,
    src: g.src,
    title: g.title || "Untitled",
    category: g.category || "Architecture",
    size: g.size || "regular",
    order: i,
...
// In-memory fallback cache, used when fs writes fail (e.g. read-only
// production filesystem). Keeps the app functional for the lifetime
// of the running process even if data can't persist to disk.
const memCache = {};
...
function collection(name, seeder) {
  return {
    get: () => {
      ...
      const seeded = seeder();
      const wrote = writeJson(name, seeded);
      if (!wrote) memCache[name] = seeded;
      return seeded;
    },
    set: (list) => {
      const wrote = writeJson(name, list);
      if (!wrote) memCache[name] = list;
      else delete memCache[name];
      return list;
    },
  };
}
...
const gallery = collection("gallery", seedGallery);
...
export const getGallery = () => gallery.get();
export const saveGallery = (list) => gallery.set(list);
...
export function logActivity({ type, action, title, detail = "" }) {
  try {
    ...
    // Never let an activity-log failure break the calling request
    // (e.g. login, save-article, etc).
    console.error("[store] logActivity failed:", err.message);
  }
}
...
export async function getSiteArticles() {
  try {
    ...
    const useDb = dbMod.useDb;
    const db = await useDb();
    if (db) {
      const ArticleMod = await import("./models/Article");
      const Article = ArticleMod.default || ArticleMod.Article;
      ...
      if (docs && docs.length > 0) {
        return docs
          .map((a) => ({
            ...
            publishedAt: a.publishedAt || a.createdAt || a.updatedAt,
          }))
          .map(toSiteArticle);
      }
    }
  } catch (err) {
    console.error("[store] getSiteArticles fell back to JSON seed:", err.message);
  }
  return getArticles().filter((a) => a.status === "published").map(toSiteArticle);
}

/** Public site view of the admin-managed gallery. Falls back to the
 * static /public folders when the store has no images yet. */
export async function getSiteGallery() {
  try {
    const dbMod = await import("./config/db");
    const db = await dbMod.useDb();
    if (db) {
      const GalleryMod = await import("./models/Gallery");
      const Gallery = GalleryMod.default || GalleryMod.Gallery;
      const docs = await Gallery.find().sort({ order: 1, createdAt: 1 }).lean();
      if (docs && docs.length > 0) return docs;
    }
  } catch (err) {
    console.error("[store] getSiteGallery fell back to JSON seed:", err.message);
  }
  return getGallery();
}
...
export function touchMember(id, lastActive = new Date().toISOString()) {
  try {
    ...
    console.error("[store] touchMember failed:", err.message);
  }
}